...
/**
 * Team-page password reset for a user's SSO login. Two paths, mirroring the platform
 * Users page's own admin actions (auth-api's AdminResetPassword /
...
export function ResetPasswordDialog({
  ...
  user: { auth_service_user_id: string; email: string } | null;
  open: boolean;
  onClose: () => void;
  ...
  async function handleSendEmail() {
    setSending(true);
    try {
      await adminSendPasswordResetEmail(accessToken, user!.auth_service_user_id);
  ...
  async function handleSetPassword() {
    ...
    setSending(true);
    try {
      await adminResetPassword(accessToken, user!.auth_service_user_id, password);
  ...
  async function handleGenerate() {
    setSending(true);
    try {
      const res = await adminResetPassword(accessToken, user!.auth_service_user_id);
    ...
    <div className="fixed inset-0 z-[60] flex items-center justify-center">
      ...
      <div className="relative z-[61] w-full max-w-sm mx-4 bg-card border border-border rounded-2xl shadow-xl p-5 space-y-4">
        <div className="flex items-center justify-between">
          <h3 className="font-bold text-sm">Reset password — {user.email}</h3>
        ...
        {tempPassword ? (
          <div className="space-y-3">
            <p className="text-xs text-muted-foreground">
              Share this temporary password with {user.email} — it won&apos;t be shown again. They&apos;ll be asked to change it at next login.
            </p>
          ...
          <div className="space-y-3">
            ...
            <div className="flex gap-2 pt-1">
              <Button variant="outline" className="flex-1" onClick={() => setMode('choose')} disabled={sending}>
                Back
              </Button>
              <Button className="flex-1" onClick={handleSetPassword} disabled={sending}>
