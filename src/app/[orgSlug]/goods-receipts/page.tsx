                                ...
                                {rows.map((g) => (
                                    <tr key={g.id} className="border-b border-border hover:bg-muted/20">
                                        ...
                                        <td className="px-6 py-3 text-right">
                                            {canChange && g.status === 'draft' && (
                                                <Button variant="outline" size="sm" disabled={post.isPending} onClick={() => post.mutate(g.id, { onSuccess: () => toast.success('GRN posted — stock updated'), onError: () => toast.error('Failed to post GRN') })}>Post</Button>
                                            )}
                                        </td>
                                    </tr>
                                ))}
